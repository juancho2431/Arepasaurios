...
const Compras = () => {
  ...
  const actualizarStockIngrediente = async () => {
    try {
      ...
      const response = await axios.put(`http://localhost:3000/api/ingredientes/${stockIngrediente.id}`, {
  ...
  const actualizarStockBebida = async () => {
    try {
      ...
      const response = await axios.put(`http://localhost:3000/api/bebidas/${stockBebida.id}`, {
